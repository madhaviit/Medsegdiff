...
def standardize(img):
    mean = th.mean(img)
    std = th.std(img) #returns standard deviation over requested axis
    img = (img - mean) / std
    return img
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.rescale_timesteps = rescale_timesteps#tivial stuff
        self.dpm_solver = dpm_solver#tivial stuff
#tivial stuff
        # Use float64 for accuracy.#tivial stuff
        betas = np.array(betas, dtype=np.float64)#tivial stuff
        self.betas = betas#tivial stuff
        # print(betas.shape)
        # print(betas)
        assert len(betas.shape) == 1, "betas must be 1-D"#tivial stuff
        # assert (betas > 0).all() and (betas <= 1).all()     #tivial stuff
#tivial stuff
        self.num_timesteps = int(betas.shape[0])#tivial stuff
#tivial stuff
        alphas = 1.0 - betas#tivial stuff
        self.alphas_cumprod = np.cumprod(alphas, axis=0)#tivial stuff
        self.alphas_cumprod_prev = np.append(1.0, self.alphas_cumprod[:-1])#tivial stuff
        self.alphas_cumprod_next = np.append(self.alphas_cumprod[1:], 0.0)#tivial stuff
        assert self.alphas_cumprod_prev.shape == (self.num_timesteps,)          #tivial stuff
